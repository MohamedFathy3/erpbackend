...
return new class extends Migration {
    public function up(): void
    {
        ...
        $permissions = [
            ['crm.view', 'View CRM', 'عرض CRM', 'crm'],
            ['crm.manage_leads', 'Manage Leads', 'إدارة العملاء المحتملين', 'crm'],
            ['crm.manage_deals', 'Manage Deals', 'إدارة الصفقات', 'crm'],
            ['crm.view_reports', 'View CRM Reports', 'عرض تقارير CRM', 'crm'],
            ['crm.send_email', 'Send CRM Email', 'إرسال بريد CRM', 'crm'],
            ['users.view', 'View Users', 'عرض المستخدمين', 'users'],
            ['users.manage', 'Manage Users', 'إدارة المستخدمين', 'users'],
            ['roles.manage', 'Manage Roles', 'إدارة الأدوار والصلاحيات', 'users'],
            ['notifications.view', 'View Notifications', 'عرض الإشعارات', 'system'],
            ['settings.manage', 'Manage Settings', 'إدارة الإعدادات', 'system'],
        ];
        foreach ($permissions as $permission) {
            DB::table('permissions')->insertOrIgnore(['key' => $permission[0], 'name' => $permission[1], 'name_ar' => $permission[2], 'module' => $permission[3], 'created_at' => now(), 'updated_at' => now()]);
        }
