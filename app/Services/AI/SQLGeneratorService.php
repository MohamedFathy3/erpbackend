...
PROMPT;
        $conversation = array_slice(array_map(static fn ($item) => [
            'role' => $item['role'] ?? 'user',
            'content' => mb_substr((string) ($item['content'] ?? ''), 0, 800),
        ], $history), -4);
        return $this->gemini->json($system, ['schema' => $this->schema->getSchema(), 'question' => mb_substr($question, 0, 4000), 'conversation' => $conversation], ['intent' => 'string', 'sql' => 'string|null', 'parameters' => 'object', 'tables' => 'array', 'clarification_needed' => 'string|null']);
    }
}
