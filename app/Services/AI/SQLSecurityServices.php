...
class SQLValidatorService
{
    ...
    public function validate(?string $sql, array $parameters, array $schema, mixed $tenantId = null): string
    {
        ...
        foreach ($schema['tables'] ?? [] as $table) {
            if (in_array('tenant_id', array_column($table['columns'] ?? [], 'name'), true)) $tenantTables[] = $table['table'];
        }
        if (array_intersect($matches[1], $tenantTables)) {
            if ($tenantId === null || !array_key_exists('tenant_id', $parameters) || !preg_match('/\btenant_id\s*=\s*:tenant_id\b/i', $sql)) throw new RuntimeException('Tenant isolation is required for this query.');
        }
        ...
        if (preg_match('/:([a-zA-Z_][a-zA-Z0-9_]*)/', $sql, $placeholders)) {
