...
class AdvancedAccessController extends Controller
{
    ...
    public function roles(Request $request) { abort_unless($this->canManage($request), 403); return response()->json(['data'=>Role::with('permissions')->whereRaw("lower(name) not in ('admin', 'administrator', 'super admin', 'super administrator')")->orderBy('name')->get()]); }
