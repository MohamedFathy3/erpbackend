...
class TrialManagementController extends Controller
{
    ...
    public function index(Request $request) { $this->guard($request); $items=Tenant::withoutGlobalScopes()->where(function ($query) { $query->where('subscription_status','trial')->orWhere('status','suspended')->orWhere('subscription_status','suspended'); })->latest()->get()->map(fn($tenant)=>$this->view($tenant)); return response()->json(['data'=>$items]); }
